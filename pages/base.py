...
class Base:
    ...
    def click_last_element(self, locator: str): #кликаем по первому элементу, если по индексу выдает out of range
    ...
    def get_element_dropdown_list(self, value_text):
        ...
        # time.sleep(1)
        # input_id[0].press('Enter')
        dropdown_menu = self.page.query_selector('.ant-select-dropdown')
        item_found = False
        ...
        while not item_found:
            ...
            if item:
                item_found = True
                item.click()  # Клик по найденному элементу
            else:
                # Скроллим вниз до конца контейнера
                dropdown_menu.hover()
                self.page.mouse.wheel(0, 500)

                # Ожидаем подгрузки новых элементов (можно настроить таймаут)
